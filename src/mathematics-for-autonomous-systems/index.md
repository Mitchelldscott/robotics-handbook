...
the four fundamental subspaces; eigenvalues and eigenvectors; diagonalization
and Jordan normal form; orthogonality and projections; least-squares fitting;
...
-   **Dynamic Mode Decomposition**: Recover the discrete linear dynamics of a
    system by exciting it and measuring the state.
...
variance, and covariance; independence and conditional independence; entropy and
mutual information; relative entropy (KL divergence); Fisher information and the
Cramér-Rao bound; parameter estimation (maximum likelihood, MAP); Bayes filters
and recursive estimation; Kalman filters and Extended Kalman Filters; particle
filters and Monte Carlo localization.

You cannot fuse sensor data, estimate hidden state, or make principled decisions
under uncertainty without probability theory. It is essential for perception,
...
(Gaussian elimination, iterative methods, LU decomposition); eigenvalue
computation (power iteration, QR algorithm); numerical integration of ODEs
...
Without robust numerics, elegant mathematical solutions will fail silently or
oscillate. Proper integration of nonlinear dynamics, correct handling of stiff
equations, and stable root-finding are essential for real-time control.
...
learning (k-means, GMM, PCA); dimensionality reduction (t-SNE, autoencoders);
